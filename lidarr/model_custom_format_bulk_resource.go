/*
Lidarr

Lidarr API docs

API version: v3.1.0.4875
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package lidarr

import (
	"encoding/json"
)

// checks if the CustomFormatBulkResource type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &CustomFormatBulkResource{}

// CustomFormatBulkResource struct for CustomFormatBulkResource
type CustomFormatBulkResource struct {
	Ids []int32 `json:"ids,omitempty"`
	IncludeCustomFormatWhenRenaming NullableBool `json:"includeCustomFormatWhenRenaming,omitempty"`
}

// NewCustomFormatBulkResource instantiates a new CustomFormatBulkResource object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewCustomFormatBulkResource() *CustomFormatBulkResource {
	this := CustomFormatBulkResource{}
	return &this
}

// NewCustomFormatBulkResourceWithDefaults instantiates a new CustomFormatBulkResource object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewCustomFormatBulkResourceWithDefaults() *CustomFormatBulkResource {
	this := CustomFormatBulkResource{}
	return &this
}

// GetIds returns the Ids field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *CustomFormatBulkResource) GetIds() []int32 {
	if o == nil {
		var ret []int32
		return ret
	}
	return o.Ids
}

// GetIdsOk returns a tuple with the Ids field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *CustomFormatBulkResource) GetIdsOk() ([]int32, bool) {
	if o == nil || IsNil(o.Ids) {
		return nil, false
	}
	return o.Ids, true
}

// HasIds returns a boolean if a field has been set.
func (o *CustomFormatBulkResource) HasIds() bool {
	if o != nil && !IsNil(o.Ids) {
		return true
	}

	return false
}

// SetIds gets a reference to the given []int32 and assigns it to the Ids field.
func (o *CustomFormatBulkResource) SetIds(v []int32) {
	o.Ids = v
}

// GetIncludeCustomFormatWhenRenaming returns the IncludeCustomFormatWhenRenaming field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *CustomFormatBulkResource) GetIncludeCustomFormatWhenRenaming() bool {
	if o == nil || IsNil(o.IncludeCustomFormatWhenRenaming.Get()) {
		var ret bool
		return ret
	}
	return *o.IncludeCustomFormatWhenRenaming.Get()
}

// GetIncludeCustomFormatWhenRenamingOk returns a tuple with the IncludeCustomFormatWhenRenaming field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *CustomFormatBulkResource) GetIncludeCustomFormatWhenRenamingOk() (*bool, bool) {
	if o == nil {
		return nil, false
	}
	return o.IncludeCustomFormatWhenRenaming.Get(), o.IncludeCustomFormatWhenRenaming.IsSet()
}

// HasIncludeCustomFormatWhenRenaming returns a boolean if a field has been set.
func (o *CustomFormatBulkResource) HasIncludeCustomFormatWhenRenaming() bool {
	if o != nil && o.IncludeCustomFormatWhenRenaming.IsSet() {
		return true
	}

	return false
}

// SetIncludeCustomFormatWhenRenaming gets a reference to the given NullableBool and assigns it to the IncludeCustomFormatWhenRenaming field.
func (o *CustomFormatBulkResource) SetIncludeCustomFormatWhenRenaming(v bool) {
	o.IncludeCustomFormatWhenRenaming.Set(&v)
}
// SetIncludeCustomFormatWhenRenamingNil sets the value for IncludeCustomFormatWhenRenaming to be an explicit nil
func (o *CustomFormatBulkResource) SetIncludeCustomFormatWhenRenamingNil() {
	o.IncludeCustomFormatWhenRenaming.Set(nil)
}

// UnsetIncludeCustomFormatWhenRenaming ensures that no value is present for IncludeCustomFormatWhenRenaming, not even an explicit nil
func (o *CustomFormatBulkResource) UnsetIncludeCustomFormatWhenRenaming() {
	o.IncludeCustomFormatWhenRenaming.Unset()
}

func (o CustomFormatBulkResource) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o CustomFormatBulkResource) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if o.Ids != nil {
		toSerialize["ids"] = o.Ids
	}
	if o.IncludeCustomFormatWhenRenaming.IsSet() {
		toSerialize["includeCustomFormatWhenRenaming"] = o.IncludeCustomFormatWhenRenaming.Get()
	}
	return toSerialize, nil
}

type NullableCustomFormatBulkResource struct {
	value *CustomFormatBulkResource
	isSet bool
}

func (v NullableCustomFormatBulkResource) Get() *CustomFormatBulkResource {
	return v.value
}

func (v *NullableCustomFormatBulkResource) Set(val *CustomFormatBulkResource) {
	v.value = val
	v.isSet = true
}

func (v NullableCustomFormatBulkResource) IsSet() bool {
	return v.isSet
}

func (v *NullableCustomFormatBulkResource) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableCustomFormatBulkResource(val *CustomFormatBulkResource) *NullableCustomFormatBulkResource {
	return &NullableCustomFormatBulkResource{value: val, isSet: true}
}

func (v NullableCustomFormatBulkResource) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableCustomFormatBulkResource) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


